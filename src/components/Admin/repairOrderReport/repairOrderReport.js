...
const RepairOrderReport = () => {
  ...
  /** 🔹 Only repair payments */
  const repairPaymentLog = paymentLogs.filter(
    (payment) => payment.transactionType === "repair-order-payment"
  );

  /** 🔹 Search + Filter */
  const filteredLogs = repairPaymentLog.filter((item) => {
